...
function writeDistribution(directory, manifest) {
  ...
  fs.writeFileSync(
    ...
    `<main data-manifest-id="${manifest.manifestId}">Proyecto externo</main>\n`,
    "utf8"
  );
  fs.writeFileSync(
...
function createFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "variamos-dspl-nginx-"));
...
test("nginx-container-v1 publica solo en loopback y deja una release trazable e idempotente", async () => {
...
test("nginx-container-v1 restaura la release anterior si el candidato no inicia", async () => {
  const fixture = createFixture();
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.match(error.message, /se restauró la liberación previa exitosa/);
      return true;
    }
  );
...
test("nginx-container-v1 puede restaurar explícitamente la release previa exitosa", async () => {
  const fixture = createFixture();
  const firstManifest = createManifest("001");
  writeDistribution(fixture.dist, firstManifest);
  const first = await fixture.deployer.deploy(deploymentRequest(fixture, firstManifest));
...
test("nginx-container-v1 rechaza un directorio que no proviene del manifest indicado", async () => {
  const fixture = createFixture();
  ...
  await assert.rejects(
    ...
    /build-metadata\.json no corresponde/
  );
