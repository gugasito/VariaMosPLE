...
function assertStableIdentifier(value: string, field: string): void {
  if (!/^[a-z0-9]+(?:[._-][a-z0-9]+)*$/.test(value)) {
    throw new NginxContainerDeploymentError(
      `Field '${field}' is not a valid stable identifier.`
    );
  }
}
...
function assertPort(port: number): void {
  if (!Number.isInteger(port) || port < 1024 || port > 65535) {
    throw new NginxContainerDeploymentError(
      "The local port must be an integer between 1024 and 65535."
    );
  }
}
...
function assertImage(image: string): void {
  if (!image || /\s/.test(image) || image.length > 512) {
    throw new NginxContainerDeploymentError(
      "The Nginx image must be a non-empty Docker reference without spaces."
    );
  }
}
...
function createContainerName(targetId: string, releaseId: string): string {
  ...
  return `variamos-spl-${targetPart}-${suffix}`.slice(0, 63);
}
...
async function defaultHealthChecker(request: HealthCheckRequest): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const httpRequest = http.get(request.url, { timeout: request.timeoutMs }, (response) => {
      ...
      response.on("end", () => {
        ...
        if (statusCode < 200 || statusCode >= 400) {
          reject(
            new NginxContainerDeploymentError(
              `Health check returned HTTP ${statusCode} at '${request.url}'.`
            )
          );
          return;
        }
        ...
        if (!body.includes(marker)) {
          reject(
            new NginxContainerDeploymentError(
              `Health check responded but does not match manifest '${request.expectedManifestId}'.`
            )
          );
          return;
        }
    ...
    httpRequest.on("timeout", () => {
      httpRequest.destroy(
        new NginxContainerDeploymentError(`Health check timed out at '${request.url}'.`)
      );
    });
...
export class NginxContainerDeployer {
  ...
  public async deploy(
    ...
    try {
      ...
      this.docker.run([
        ...
        containerName,
        "--label",
        `variamos.spl.manifest-id=${request.manifest.manifestId}`,
        "--label",
        `variamos.spl.target-id=${request.manifest.target.id}`,
        "--publish",
      ...
      const recoveryDetails = recovery.length > 0 ? ` Recovery: ${recovery.join(" ")}` : "";
      throw new NginxContainerDeploymentError(
        `Could not deploy '${request.manifest.manifestId}': ${asErrorMessage(error)}.${recoveryDetails}`
      );
    }
  }
  ...
  public async rollback(request: NginxRollbackRequest): Promise<NginxRollbackResult> {
    ...
    if (!replacedRelease) {
      throw new NginxContainerDeploymentError(
        `No active release is registered for '${request.targetId}'.`
      );
    }
    if (!replacedRelease.previousReleaseId) {
      throw new NginxContainerDeploymentError(
        `Release '${replacedRelease.releaseId}' has no previous release to restore.`
      );
    }
    if (replacedRelease.endpoint.port !== request.hostPort) {
      throw new NginxContainerDeploymentError(
        "The requested port does not match the registered active release."
      );
    }
    ...
    if (!activeRelease) {
      throw new NginxContainerDeploymentError(
        `Previous release record '${replacedRelease.previousReleaseId}' was not found.`
      );
    }
    ...
    try {
      ...
      if (restoredStarted) {
        try {
          ...
          recovery.push("stopped the previous release that failed its health check.");
        } catch (stopError) {
          recovery.push(`could not stop the previous release: ${asErrorMessage(stopError)}.`);
        }
      }
      if (replacementStopped) {
        try {
          this.docker.run(["start", replacedRelease.containerName]);
          recovery.push("restored the release that was active before rollback.");
        } catch (startError) {
          recovery.push(`could not restore the original release: ${asErrorMessage(startError)}.`);
        }
      }
      throw new NginxContainerDeploymentError(
        `Could not roll back '${request.targetId}': ${asErrorMessage(error)}. ${recovery.join(" ")}`
      );
    }
  }
  ...
  private assertDeploymentPlan(manifest: DeploymentManifest): { healthPath: string } {
    ...
    if (!hasNginxDeploy || !hasHttpVerification || !healthCheck?.path) {
      throw new NginxContainerDeploymentError(
        "The manifest does not declare the deploy/verify plan required by nginx-container-v1."
      );
    }
    if (!healthCheck.path.startsWith("/") || healthCheck.path.startsWith("//")) {
      throw new NginxContainerDeploymentError(
        "The health-check path must be relative to the HTTP server."
      );
    }
    if (manifest.rollback.strategy !== "previous-successful-release") {
      throw new NginxContainerDeploymentError(
        "nginx-container-v1 requires the 'previous-successful-release' rollback strategy."
      );
    }
  ...
  private assertDistribution(
    ...
    try {
      ...
      throw new NginxContainerDeploymentError(
        `Could not access the distribution directory: ${asErrorMessage(error)}`
      );
    }
    ...
    if (!fs.statSync(directory).isDirectory()) {
      throw new NginxContainerDeploymentError("The distribution path is not a directory.");
    }
    if (!fs.existsSync(indexPath) || !fs.existsSync(metadataPath)) {
      throw new NginxContainerDeploymentError(
        "The distribution directory must contain index.html and build-metadata.json."
      );
    }
    if (!fs.lstatSync(indexPath).isFile() || !fs.lstatSync(metadataPath).isFile()) {
      throw new NginxContainerDeploymentError(
        "index.html and build-metadata.json must be regular files, not links."
      );
    }
    ...
    try {
      ...
      throw new NginxContainerDeploymentError(
        `Could not read build-metadata.json: ${asErrorMessage(error)}`
      );
    }
    if (!this.matchesBuildMetadata(metadata, manifest)) {
      throw new NginxContainerDeploymentError(
        "build-metadata.json does not match the manifest being deployed."
      );
    }
  ...
  private createReleaseDistributionSnapshot(
    ...
    try {
      ...
      throw new NginxContainerDeploymentError(
        `Could not create the immutable snapshot for release '${releaseId}': ${asErrorMessage(error)}`
      );
    }
  ...
  private readRelease(releasePath: string): NginxReleaseRecord | undefined {
    ...
    try {
      ...
      throw new NginxContainerDeploymentError(
        `Release record '${releasePath}' is invalid: ${asErrorMessage(error)}`
      );
    }
  }
  ...
  private assertReleaseRecord(release: NginxReleaseRecord): void {
    if (
      ...
      throw new NginxContainerDeploymentError("The release record does not comply with nginx-container-release/v1.");
    }
  }
  ...
  private async waitForHealthy({
    ...
    throw new NginxContainerDeploymentError(
      `The health check did not confirm '${expectedManifestId}' at '${url}': ${asErrorMessage(lastError)}`
    );
  }
  ...
  private async recoverPreviousRelease({
    ...
    if (candidateCreated) {
      try {
        ...
        recovery.push("removed the candidate that failed verification.");
      } catch (error) {
    ...
    if (previousStopped && previousRelease) {
      try {
        ...
        recovery.push("restored the previous successful release.");
      } catch (error) {
        recovery.push(`could not restore the previous release: ${asErrorMessage(error)}.`);
      }
    }
    return recovery;
  }
