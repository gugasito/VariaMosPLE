...
The local server lets the UI request a derivation without receiving access to Docker, Git, SSH, or secrets:

```bash
npm run serve:orchestrator
```

It listens only on `127.0.0.1:8090` by default and exposes:
...
Onboarding keeps its state outside the repository—under `/tmp/variamos-spl-external-projects` by default. For Git, it uses managed checkouts that never alter the development checkout: a branch or tag is resolved to a commit before `.variamos/spl.json` is read. For a folder without Git, it copies the descriptor and declared artifacts to a managed snapshot identified by SHA-256. Both paths produce an immutable catalog consumed by the existing pipeline.

`GET /api/spl/v1/providers` publishes only the catalog used by the **Source type** selector: remote Git, local Git, and local folder without Git. Local Git is available only when the operator enables `SPL_ALLOW_LOCAL_GIT_REPOSITORIES=true`; the folder without Git requires `SPL_ALLOW_LOCAL_DIRECTORIES=true`.
...
HTTPS and SSH repositories are enabled. Local Git paths are blocked unless the operator sets `SPL_ALLOW_LOCAL_GIT_REPOSITORIES=true`, and non-Git folders are blocked unless `SPL_ALLOW_LOCAL_DIRECTORIES=true`. The API requires absolute host paths, rejects escapes and symbolic links outside the root, and verifies that the snapshot has not changed between preview and save. A private repository must use a `credentialRef` plus a Git/SSH credential mechanism managed by the operator; the credential itself neither crosses the API nor persists in the model.
