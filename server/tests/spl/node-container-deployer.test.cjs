...
} = require("../../dist/spl/adapters/deployers/NodeContainerDeployer.js");
...
test("node-container-v1 creates a traceable release and requires the manifest health check", async () => {
  ...
  try {
    ...
    assert.equal(docker.calls.some((args) => args.includes(`SPL_MANIFEST_ID=${manifest.manifestId}`)), true);
    assert.equal(docker.calls.some((args) => args.includes(`variamos.spl.target-id=${manifest.target.id}`)), true);
  } finally {
