...
} = require("../../dist/spl/adapters/providers/ArtifactProvider.js");
const {
  GitArtifactProvider,
} = require("../../dist/spl/adapters/providers/GitArtifactProvider.js");
const {
  LocalArtifactProvider,
} = require("../../dist/spl/adapters/providers/LocalArtifactProvider.js");
const {
  StaticSiteBuilder,
  StaticSiteBuildError,
} = require("../../dist/spl/adapters/builders/StaticSiteBuilder.js");
...
function createGitFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "variamos-spl-git-"));
  const remote = "https://example.invalid/project-assets.git";
  const relativePath = "assets/card.html";
  const content = Buffer.from('<article class="card">External project</article>\n', "utf8");

  execFileSync("git", ["init", root], { stdio: "ignore" });
  execFileSync("git", ["-C", root, "config", "user.email", "tests@example.invalid"]);
  execFileSync("git", ["-C", root, "config", "user.name", "SPL Tests"]);
...
function createBuildFixture() {
  ...
  return {
    ...
    manifest,
    providers,
    outputDirectory: fs.mkdtempSync(path.join(os.tmpdir(), "variamos-spl-build-")),
  };
}

test("the Git provider reads a file from an exact commit without changing the checkout", () => {
...
test("the local provider reads artifacts only within registered roots", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "variamos-spl-local-"));
...
test("the static builder verifies the digest and builds index.html with metadata", () => {
  const fixture = createBuildFixture();
  ...
  assert.match(html, /External project/);
...
test("the builder rejects content that does not match the manifest digest", () => {
  const fixture = createBuildFixture();
  ...
  assert.throws(
    ...
    (error) => {
      ...
      assert.match(error.message, /differs between the catalog and manifest/);
      return true;
    }
  );
});
