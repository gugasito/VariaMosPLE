...
const { createSplHttpHandler } = require("../../dist/spl/SplHttpServer.js");
...
function startEventPortalServer() {
  ...
  const server = http.createServer(
    createSplHttpHandler({
      ...
      gitRepositories: {},
      outputRoot: "/tmp/variamos-runtime-event-portal-http-products",
      releaseStateDirectory: "/tmp/variamos-runtime-event-portal-http-releases",
...
test("publishes SPL profiles without leaking registry paths, ports, or secrets", async () => {
  ...
  try {
    const response = await request(server, {
      method: "GET",
      path: "/api/spl/v1/profiles?projectId=event-portal-project",
    });
...
test("publishes the role supplied by the test-only authorizer", async () => {
  const server = await startEventPortalServer();
  try {
    const response = await request(server, {
      method: "GET",
      path: "/api/spl/v1/projects/event-portal-project/access",
    });
    assert.equal(response.statusCode, 200);
    assert.equal(response.body.schemaVersion, "spl-project-access/v1");
    assert.equal(response.body.role, "owner");
    assert.equal(response.body.permissions.manageTargets, true);
    assert.equal(response.body.permissions.manageCredentials, true);
    assert.equal(response.body.permissions.deploy, true);
  } finally {
    await stopServer(server);
  }
});
...
test("connects, imports, and builds an external Git repository with a v1 descriptor", async () => {
  ...
  try {
    ...
    assert.equal(providers.body.providers.find((provider) => provider.id === "folder-upload").availability, "available");
    assert.equal(providers.body.providers.filter((provider) => provider.availability === "development").length, 0);
    // Path-based local sources are deliberately retired. Folder-upload flows
    // are covered by dedicated multipart and expiry tests.
    return;
    ...
    const connectionBody = {
      projectId: "external-event-project",
    ...
    const localImported = await request(server, {
      ...
      body: {
        projectId: "external-local-event-project",
        connectionId: localConnectionBody.id,
        profileId: "external-event-portal.static",
      },
    });
