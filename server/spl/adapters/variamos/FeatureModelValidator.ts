...
/** Deliberately small local validator for the Feature Model subset used by
 * VariaMos. It avoids delegating deployment safety to a remote semantic
 * service. */
export function validateFeatureModel(model: VariaMosSerializedModel): string[] {
  ...
  if (roots.length !== 1) errors.push("The feature model must declare exactly one RootFeature.");
  if (roots[0] && selected(roots[0].id) !== true) errors.push("The root feature must be selected.");
  ...
  model.elements.filter((element) => "Selected" in Object.fromEntries((element.properties || []).map((item) => [item.name, item.value]))).forEach((element) => {
    if (selected(element.id) === undefined) errors.push(`Feature '${named(element.id)}' must be Selected or Unselected.`);
  });

  for (const relationship of model.relationships || []) {
    const source = byId.get(relationship.sourceId);
    const target = byId.get(relationship.targetId);
    if (!source || !target) { errors.push(`Relationship '${relationship.id}' points to a missing feature.`); continue; }
    ...
    if (type === "Mandatory") {
      if (sourceSelected === true && target.type !== "Bundle" && targetSelected !== true) errors.push(`'${named(target.id)}' is mandatory when '${named(source.id)}' is selected.`);
      if (target.type !== "Bundle" && targetSelected === true && sourceSelected !== true) errors.push(`'${named(target.id)}' requires '${named(source.id)}' to be selected.`);
    } else if (type === "Optional") {
      if (targetSelected === true && sourceSelected !== true) errors.push(`'${named(target.id)}' cannot be selected without '${named(source.id)}'.`);
    } else if (type === "Includes" && sourceSelected === true && targetSelected !== true) {
      errors.push(`'${named(source.id)}' includes '${named(target.id)}'.`);
    } else if (type === "Excludes" && sourceSelected === true && targetSelected === true) {
      errors.push(`'${named(source.id)}' excludes '${named(target.id)}'.`);
    }
  }
  ...
  for (const bundle of model.elements.filter((element) => element.type === "Bundle")) {
    ...
    if (type === "Xor" && count !== 1) errors.push(`XOR group '${named(bundle.id)}' requires exactly one alternative.`);
    if (type === "Or" && count < 1) errors.push(`OR group '${named(bundle.id)}' requires at least one alternative.`);
    if (type === "And" && count !== children.length) errors.push(`AND group '${named(bundle.id)}' requires all of its alternatives.`);
    if (type === "Range") {
      const min = Number(property(bundle, "RangeMin"));
      const max = Number(property(bundle, "RangeMax"));
      if (!Number.isInteger(min) || !Number.isInteger(max) || count < min || count > max) errors.push(`Range group '${named(bundle.id)}' does not satisfy its cardinality.`);
    }
  }
  return errors;
}
