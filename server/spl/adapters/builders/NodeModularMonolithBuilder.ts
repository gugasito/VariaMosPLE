...
function safeOutputPath(entrypoint: string, artifactId: string): string {
  if (!entrypoint || path.isAbsolute(entrypoint) || entrypoint.split(/[\\/]/).includes("..") || !entrypoint.startsWith("src/")) {
    throw new NodeModularMonolithBuildError(
      `Artifact '${artifactId}' does not declare a safe entrypoint under src/.`
    );
  }
  return entrypoint;
}
...
export class NodeModularMonolithBuilder {
  public build(request: NodeModularMonolithBuildRequest): NodeModularMonolithBuildResult {
    ...
    if (buildOperations.length !== 2) {
      throw new NodeModularMonolithBuildError(
        "The manifest does not contain generate/build operations for node-modular-monolith-v1."
      );
    }
    ...
    request.manifest.artifacts.forEach((manifestArtifact) => {
      ...
      if (!artifact) throw new NodeModularMonolithBuildError(`The catalog does not contain '${manifestArtifact.id}'.`);
      ...
      if (actualDigest !== artifact.integrity.digest || actualDigest !== manifestArtifact.digest) {
        throw new NodeModularMonolithBuildError(`The digest for '${artifact.id}' does not match the catalog or manifest.`);
      }
    ...
    sourceFiles.forEach((sourceFile) => {
      ...
      if (diagnostics.length > 0) {
        throw new NodeModularMonolithBuildError(
          `Could not transpile '${sourceFile}': ${diagnostics.map((item) => ts.flattenDiagnosticMessageText(item.messageText, " ")).join(" | ")}`
        );
      }
    ...
    if (!fs.existsSync(runtimePath)) {
      throw new NodeModularMonolithBuildError("The monolithic product does not contain src/runtime/server.ts.");
    }
  ...
  private assertArtifact(artifact: Artifact, manifestArtifact: DeploymentManifest["artifacts"][number]): void {
    ...
    if (artifact.version !== manifestArtifact.version || artifact.integrity.digest !== manifestArtifact.digest) {
      throw new NodeModularMonolithBuildError(`The manifest does not match the catalog for '${artifact.id}'.`);
    }
  }
